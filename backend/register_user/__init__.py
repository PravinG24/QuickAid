...
def main(req: func.HttpRequest) -> func.HttpResponse:
    logging.info("register_user function triggered.")
    ...
    check_query  = "SELECT * FROM c WHERE c.type = 'user' AND c.email = @email"
    ...
    if existing:
        return func.HttpResponse(
            json.dumps({"error": "Email is already registered as user."}),
            status_code=409,
            mimetype="application/json"
        )

    # ── Generate USR-XX ID ───────────────────────────────────────────────────
    count_query  = "SELECT VALUE COUNT(1) FROM c WHERE c.type = 'user'"
    count_result = list(container.query_items(
        query=count_query,
        enable_cross_partition_query=True
    ))
    count   = count_result[0] if count_result else 0
    user_id = f"USR-{str(count + 1).zfill(2)}"
    ...
    try:
        ...
        return func.HttpResponse(
            json.dumps({"error": "Failed to register user. Please try again later."}),
            status_code=500,
            mimetype="application/json"
        )
