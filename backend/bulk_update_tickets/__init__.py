...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    if not isinstance(ticket_ids, list) or not ticket_ids:
        return func.HttpResponse(
            json.dumps({"error": "ticketIds must be a non-empty list."}),
            status_code=400,
            mimetype="application/json"
        )
    ...
    updates = {k: v for k, v in body.items() if k in allowed_updates and v}

    if not updates:
        return func.HttpResponse(
            json.dumps({
                "error": f"At least one update field required. Allowed: {', '.join(allowed_updates)}"
            }),
            status_code=400,
            mimetype="application/json"
        )
    ...
    if "status" in updates and updates["status"] not in ["Open", "In Progress", "Resolved", "Closed"]:
        return func.HttpResponse(
            json.dumps({"error": "Invalid status. Allowed: Open, In Progress, Resolved, Closed"}),
            status_code=400,
            mimetype="application/json"
        )
    ...
    if "priority" in updates and updates["priority"] not in ["Low", "Medium", "High"]:
        return func.HttpResponse(
            json.dumps({"error": "Invalid priority. Allowed: Low, Medium, High"}),
            status_code=400,
            mimetype="application/json"
        )
    ...
    try:
        ...
        for ticket_id in ticket_ids:
            ...
            # If no actual changes, skip this ticket
            if not changed_fields:
                results.append({
                    "ticketId": ticket_id,
                    "status": "no_changes",
                    "message": "No actual field changes detected."
                })
                continue
            ...
            results.append({
                "ticketId": ticket_id,
                "status": "updated",
                "ticket": {k: v for k, v in ticket.items() if k not in {"_rid", "_self", "_etag", "_attachments", "_ts"}}
            })
    ...
    return func.HttpResponse(
        json.dumps({
            "message": f"Bulk update completed. {updated_count} of {len(ticket_ids)} ticket(s) updated.",
            "totalRequested": len(ticket_ids),
